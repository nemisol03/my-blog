...
function PostDetail() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setLoading(true);
                const postResponse = await instance.get(`posts/${slug}`);
                setLoading(false);
...
console.log(post)
    return (
      <>
           {!loading && <div className="page-container max-w-[800px] mt-16">
                <div className=" gap-x-12">
                    <div className="">
                        ...
                        <img src={post?.user?.avatar || '/user-defeault.jpg'} className="w-10 h-10 object-cover rounded-full" alt="user's avatar"/>
                        <div className='mb-6'>
                            ...
                            <p className="text-gray-600 text-xs">{post?.created_at && formatDistanceToNow(post.created_at)}</p>
                        </div>
            ...
            {loading && <div className='flex justify-center items-center h-screen'><MoonLoader size={50} color='#4B5563'/></div>}
      </>
    );
}
